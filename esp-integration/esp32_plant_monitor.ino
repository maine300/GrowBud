...
void loop() {
  ...
  // Update status LED
  digitalWrite(STATUS_LED_PIN, WiFi.status() == WL_CONNECTED);
...
void readAndSendSensorData() {
  ...
  int soilMoisture = map(soilRaw, 4095, 0, 0, 100); // Invert and convert to percentage
...
void updateDeviceOnServer(String deviceId, bool state) {
  ...
  HTTPClient http;
  String url = String(serverURL) + "/api/devices/" + deviceId;
  http.begin(url);
  http.addHeader("Content-Type", "application/json");
